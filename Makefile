...
run:
	-nvidia-docker rm $(REPO)
	nvidia-docker run -d --expose 5000 -e PYTHONUNBUFFERED=1 --network="host" --privileged --volume=/dev:/dev -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=:0 -e QT_X11_NO_MITSHM=1 --name=$(REPO) $(IMNAME)

launch:
ifeq (${DEVICE}, gpu)
	-nvidia-docker rm $(REPO)
	nvidia-docker run --expose 5000 -e PYTHONUNBUFFERED=1 --network="host" --privileged --volume=/dev:/dev -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=:0 -e QT_X11_NO_MITSHM=1 --name=$(REPO) $(IMNAME)
else
	-nvidia-docker rm $(REPO)
